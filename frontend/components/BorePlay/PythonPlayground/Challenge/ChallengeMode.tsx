...
export default function ChallengeMode() {
  ...
  const [code, setCode] = useState(
    pythonChallenges[0]?.starterCode ?? ""
  );
  ...
  const challenge = pythonChallenges[currentIndex];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.correct) {
        ...
        /*
         * Move to the next challenge only
         * after the current challenge is correct.
         */
        if (nextIndex < pythonChallenges.length) {
          setTimeout(() => {
            setCurrentIndex(nextIndex);

            setCode(
              pythonChallenges[nextIndex].starterCode
            );
